...
namespace Typewriter.Lexing
{
    public class CodeLexer
    {
        ...
        private Stack<Context> _context;
        ...
        public void Tokenize(SemanticModel semanticModel, string code/*, ProjectItem templateProjectItem*/)
        {
            ...
            _context = new Stack<Context>(new[] { _fileContext });
            ...
            semanticModel.TempIdentifiers.Add(semanticModel.ShadowClass.GetIdentifiers(_contexts));
        }
        ...
        private void ParseUsings(Stream stream)
        {
            ...
            while (true)
            {
                ...
                if ((stream.Current == 'u' && stream.PeekWord() == "using") || (stream.Current == '/' && stream.Peek() == '/'))
                {
                    ...
                    _semanticModel.ShadowClass.AddUsing(line, stream.Position);
        ...
        private void ParseDollar(Stream stream)
        {
            if (stream.Current == '$')
            {
                ...
                if (identifier != null)
                {
                    ...
                    {
                        _context.Push(_contexts.Find(identifier.Context));
                        ParseFilter(stream);
                    ...
                    {
                        var current = _context.Pop();

                        ParseBlock(stream); // template

                        _context.Push(current);
                    }
