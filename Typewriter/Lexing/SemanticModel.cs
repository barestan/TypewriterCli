...
namespace Typewriter.Lexing
{
    public class SemanticModel
    {
        ...
        public IEnumerable<Identifier> GetIdentifiers(int position)
        {
            ...
            if (contextSpan != null)
            {
                if (contextSpan.Type == ContextType.Template)
                {
                    ...
                    var extensionIdentifiers = _shadowClass.Snippets.Where(s => s.Type == SnippetType.Using && s.Code.StartsWith("using"))
                ...
                var identifiers = _shadowClass.GetRecommendedSymbols(position).GroupBy(s => s.Name).Select(g => Identifier.FromSymbol(g.First())).ToList();
        ...
        internal Identifier GetIdentifier(Context context, string name)
        {
            ...
            identifier = _tempIdentifiers.GetTempIdentifiers(context).FirstOrDefault(i => i.Name == name);
            if (identifier != null) return identifier;

            // Todo: Optimize performance
            foreach (var snippet in _shadowClass.Snippets.Where(s => s.Type == SnippetType.Using && s.Code.StartsWith("using")))
            {
