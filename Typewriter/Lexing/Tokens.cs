...
namespace Typewriter.Lexing
{
    public class Tokens
    {
        private readonly BraceStack _braces = new BraceStack();
        private readonly Dictionary<int, Token> _tokenDictionary = new Dictionary<int, Token>();

        public BraceStack BraceStack => _braces;

        public void Add(Token token)
        {
            _tokenDictionary[token.Start] = token;
        }
        ...
        internal void AddBrace(Stream stream, string classification = Classifications.Operator)
        {
            ...
            if (brace == '{' || brace == '[' || brace == '(' || brace == ')' || brace == ']' || brace == '}')
            {
                ...
                {
                    token.IsOpen = true;
                    _braces.Push(token, stream.Current);
                }
                else
                {
                    var match = _braces.Pop(brace);
                    if (match != null)
                    {
        ...
        {
            return _tokenDictionary.ContainsKey(position) ? _tokenDictionary[position] : null;
        }

        public IEnumerable<Token> FindTokens(int position)
        {
            return _tokenDictionary.Values.Where(t => t.Start <= position && t.Start + t.Length >= position);
        }
